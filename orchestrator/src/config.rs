...
///
/// 坐标系：**根（最粗）网格格子坐标**（0-based，含端点）。
/// 无论嵌套深度如何，所有层的 x_start/x_end/y_start/y_end 均以最粗网格（L0）的
/// 格子坐标为准，与 `[ibm]` 中 x0/y0/size 及结构网格文件中的坐标保持统一。
///
/// 例如：L0 为 600×100 的粗网格，L1 覆盖根坐标 [100, 280] × [20, 80]，
/// 则 `x_start=100, x_end=280, y_start=20, y_end=80`，
/// 细网格尺寸 = (280-100)×r+1 × (80-20)×r+1（其中 r=refine_ratio×累积加密比）。
#[derive(Debug, Deserialize, Clone)]
pub struct MgLevelConfig {
    /// 细化区域在**根（最粗）网格坐标系**中的 X 起始格子索引（含端点）
    pub x_start: i32,
    /// 细化区域在**根（最粗）网格坐标系**中的 X 终止格子索引（含端点）
    pub x_end: i32,
    /// 细化区域在**根（最粗）网格坐标系**中的 Y 起始格子索引（含端点）
    pub y_start: i32,
    /// 细化区域在**根（最粗）网格坐标系**中的 Y 终止格子索引（含端点）
    pub y_end: i32,
...
///
/// [[multigrid.levels]]
/// # 细化层 1：覆盖圆柱尾迹区域（根网格坐标系，所有层统一以 L0 为基准）
