...
// │  字段说明                                                                │
// ├─────────────────────────────────────────────────────────────────────────┤
// │  j_s  — 物理南边界行索引（本地网格坐标，含）                              │
// │         非 MPI：0（整个网格从 j=0 开始）                                 │
// │         1D Y-切片 MPI：1（j=0 是南幽灵行，实际物理南壁从 j=1 开始）        │
// │         2D XY 块分解：phys_y0()（= has_south_ghost ? 1 : 0）            │
// │                                                                         │
// │  j_n  — 物理北边界行索引（本地网格坐标，含）                              │
// │         非 MPI：ny-1                                                    │
// │         1D Y-切片 MPI：local_ny（= ny-2，最后一个物理行，j=ny-1 是北幽灵）│
// │         2D XY 块分解：phys_y0() + local_ny - 1                          │
// │                                                                         │
// │  i_w  — 物理西边界列索引（本地网格坐标，含）                              │
// │         非 MPI / 1D Y-切片：0                                           │
// │         2D/3D 块分解：phys_x0()（= has_west_ghost ? 1 : 0）             │
// │                                                                         │
// │  i_e  — 物理东边界列索引（本地网格坐标，含）                              │
// │         非 MPI / 1D Y-切片：nx-1                                        │
// │         2D/3D 块分解：phys_x0() + local_nx - 1                          │
// ├─────────────────────────────────────────────────────────────────────────┤
// │  这四个字段共同定义了 BC 应施加的"物理可行区间"，防止 BounceBack/ZouHe   │
// │  等 BC 被误施加到 MPI 幽灵行/列上，从而污染幽灵数据并导致接口不连续。     │
// │  apply_boundary_conditions() 在遍历壁面节点时以 j_s/j_n/i_w/i_e        │
// │  替代硬编码的 0/ny-1/0/nx-1，实现跨进程的 BC 隔离。                     │
// ├─────────────────────────────────────────────────────────────────────────┤
// │  has_*_wall 标志（默认 true）：                                          │
// │    MPI 内部分区（非全局壁面所在分区）必须设为 false，防止 BC 被误施加到   │
// │    非壁面的内部物理行/列上，导致分块边界处速度/密度出现阶跃。              │
// │    例：4 进程 1D Y 分解时，只有 rank 0 的 has_south_wall=true，           │
// │        rank 1/2/3 必须设 has_south_wall=false（已由 Solver::step() 自动  │
// │        根据 MpiDecomp::has_south_wall() 设置）。                         │
// └─────────────────────────────────────────────────────────────────────────┘
//
// 快速参考（各模式下的默认值）：
//   非 MPI         : j_s=0, j_n=ny-1, i_w=0, i_e=nx-1, all has_*_wall=true
//   1D Y-切片      : j_s=1, j_n=ny-2, i_w=0, i_e=nx-1 (has_south/north 按 rank)
//   2D XY 块分解   : j_s=phys_y0, j_n=phys_y0+local_ny-1,
//                    i_w=phys_x0, i_e=phys_x0+local_nx-1 (has_* 按 rank)
// ---------------------------------------------------------------------------
struct PhysicalBounds {
    int j_s = 0;   ///< 物理南边界行索引（含，本地坐标）；MPI 时跳过南幽灵行
    int j_n = 0;   ///< 物理北边界行索引（含，本地坐标）；MPI 时跳过北幽灵行
    int i_w = 0;   ///< 物理西边界列索引（含，本地坐标）；MPI 时跳过西幽灵列
    int i_e = 0;   ///< 物理东边界列索引（含，本地坐标）；MPI 时跳过东幽灵列
