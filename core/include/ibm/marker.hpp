...
struct Marker {
    ...
    /// 该标记点的目标速度（柔性体 / 运动刚体逐点速度）。
    /// 默认 (0,0)，适用于静止固体；运动体须在每步调用 IBM 前由外部更新。
    /// IBM 力计算函数（MDF / Penalty / MLS / IVC）均从此字段读取目标速度，
    /// 而非使用统一标量参数，从而支持柔性体（每个标记点有不同速度）。
    double ux_target = 0.0;    ///< x 方向目标速度
    double uy_target = 0.0;    ///< y 方向目标速度
};
